...
from networks.layers.show_QKV2 import process_and_visualize
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, is_return_attn_weight=False, layer_idx=0):
        ...
        # Q, K, V = Q.squeeze(1).cpu().numpy(), K.squeeze(1).cpu().numpy(), V.squeeze(1).cpu().numpy()  # 去掉维度 bs=1
        # process_and_visualize(Q, K, V)  # 可视化
