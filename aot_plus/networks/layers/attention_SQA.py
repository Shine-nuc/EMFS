...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, is_return_attn_weight=False, layer_idx=0):
        """
        ...
        if is_return_attn_weight and layer_idx == 0:

            # Scale
            Q = Q / self.T

            # Multi-head
            Q = Q.view(-1, bs, num_head, hidden_dim).permute(1, 2, 0, 3).contiguous() # 确保张量连续  # [bs head hw hd]
            K = K.view(-1, bs, num_head, hidden_dim).permute(1, 2, 0, 3).contiguous() 
            V = V.view(-1, bs, num_head, hidden_dim).permute(1, 2, 0, 3).contiguous() 
            ...
            Q_scale = (Q_max - Q_min) / (127 - (-128)) if Q_max != Q_min else 1.0  # 对于 qint8
            Q_zero_point = int(-Q_min / Q_scale + (-128))
            Q_zero_point = max(-128, min(127, Q_zero_point))  # 限制 Q_zero_point 在范围内
            

            # Q_zero_point = K_zero_point = 0
            Q_quant = torch.quantize_per_tensor(Q, scale=0.04, zero_point=Q_zero_point, dtype=torch.qint8)  # scale=0.025

            
            K_min, K_max = K.min().item(), K.max().item()
            K_scale = (K_max - K_min) / (127 - (-128)) if K_max != K_min else 1.0  # 对于 qint8
            K_zero_point = int(-K_min / K_scale + (-128))
            K_zero_point = max(-128, min(127, K_zero_point))  # 限制 K_zero_point 在范围内
            

            K_quant = torch.quantize_per_tensor(K, scale=0.04, zero_point=K_zero_point, dtype=torch.qint8)


            # 4. 提取整数表示并去零点
            Q_int = Q_quant.int_repr().int() - Q_zero_point
            K_int = K_quant.int_repr().int() - K_zero_point
            
            # print("#### Q_int.type ####" + str(Q_int.dtype))
            # print("#### K_int.type ####" + str(K_int.dtype))

            # 注意：int8矩阵乘法需要结合具体设备内核实现，这里仅提供简易做法。
            Q_float16 = Q_int.to(torch.float16)
            K_float16 = K_int.to(torch.float16)

            # 5. 矩阵乘法
            # 注意：K 需要转置以匹配矩阵乘法的维度
            result_int = torch.matmul(Q_float16, K_float16.transpose(-1, -2))

            # 6. 计算组合缩放因子
            result_scale = 0.04 * 0.04

            # 7. 反量化结果
            result_float = result_int.to(torch.float16) * result_scale

            # Activation
            attn = torch.softmax(result_float, dim=-1)
            # Dropouts
            attn = self.dropout(attn)

            # Weighted sum
            outputs = torch.matmul(attn, V).permute(2, 0, 1, 3).contiguous()  # 确保张量连续
...
class GatedPropagation(nn.Module):
    ...
    def forward(self, Q, K, V, U, size_2d, is_return_attn_weight=False, layer_idx=0):
        """
        :param Q: A 3d tensor with shape of [T_q, bs, C_q]
        :param K: A 3d tensor with shape of [T_k, bs, C_k]
        :param V: A 3d tensor with shape of [T_v, bs, C_v]
        ...
        if is_return_attn_weight and layer_idx == 0:  # 仅长期注意力的第0层需要attn矩阵（原始代码1 2层也走这里不合理）

            # print("第" + str(layer_idx) + "层用 attn ！！！")

            # Scale
            Q = Q / self.T

            # Multi-head
            Q = Q.view(-1, bs, num_head, self.d_att).permute(1, 2, 0, 3).contiguous()  # 确保张量连续  # [bs head hw hd]
            K = K.view(-1, bs, num_head, self.d_att).permute(1, 2, 0, 3).contiguous()
            V = V.view(-1, bs, num_head, hidden_dim).permute(1, 2, 0, 3).contiguous() 
        ...
        else:  # 长期注意力的1 2层和self注意力的三层都走这里的高效注意力
...
class LocalGatedPropagation(nn.Module):
    ...
    def forward(self, q, k, v, u, size_2d):
        ...
        Q_quant = torch.quantize_per_tensor(q, scale=0.025, zero_point=Q_zero_point, dtype=torch.qint8)
        K_quant = torch.quantize_per_tensor(k, scale=0.025, zero_point=K_zero_point, dtype=torch.qint8)

        Q_int = Q_quant.int_repr().int() - Q_zero_point
        K_int = K_quant.int_repr().int() - K_zero_point

        q = Q_int.to(torch.float16)
        k = K_int.to(torch.float16)
        """
        ...
        global_attn = global_attn.half()  # 使用半精度
