...
from networks.layers.attention_SQA import MultiheadAttention, GatedPropagation, LocalGatedPropagation, silu
...
from networks.layers.compute_mi import compute_redundancy

from networks.layers.show_QKV import process_and_visualize
from networks.layers.show_cos import visualize_and_save_cos_matrix
...
class LongShortTermTransformer(nn.Module):
    ...
    def restrict_long_memories(
        ...
        if use_atten_weight:
            ...
            # 冗余度计算
            global_K2 = self.layers[0].global_K2
            T, token_num, bs, embed_dim = global_K2.shape  # global_K2 的形状为 [T, token_num, bs, embed_dim]
 
            # 归一化 global_K2，沿着 token_num 维度进行归一化，这样每个 token 在 [bs, embed_dim] 维度上的向量是单位向量
            normalized = F.normalize(global_K2, p=2, dim=1)  # shape: [T, token_num, bs, embed_dim]

            # 将每个 [token_num, bs, embed_dim] 的张量展平为一个向量
            normalized_flat = normalized.view(T, -1)  # shape: [T, token_num * bs * embed_dim]

            # 计算所有张量之间的余弦相似度矩阵，通过矩阵乘法实现，结果shape: [T, T]
            cos_matrix = torch.matmul(normalized_flat, normalized_flat.t())  # .t()转置, shape: [T, T]

            # 将对角线元素设置为0，以避免自相似度影响冗余度计算
            cos_matrix.fill_diagonal_(0)

            # visualize_and_save_cos_matrix(cos_matrix)  # 可视化

            # 计算每张图片的冗余度，即每行的相似度之和
            cos_result = cos_matrix.sum(dim=1)  # shape: [T]

            # 进行归一化，使得所有冗余度之和为1
            total_sum = cos_result.sum()
            if total_sum.item() != 0:
                cos_result_normalized = (cos_result / total_sum).round(decimals=4).tolist()  # 转为列表并四舍五入
            else:
                cos_result_normalized = [0] * T  # 如果总和为0，则返回全0列表         

            # 输出结果
            print(cos_result_normalized)


            print(f"{T}###### cos_result_normalized ##### {cos_result_normalized}")

            # 转换为 NumPy 数组以便后续处理
            cos_data = np.array(cos_result_normalized)

            cos_data = 1 / (1 + np.exp(10 * (cos_data - 0.4)))  # 冗余度大于0.25开始极速惩罚
    
            print("改变后的数据1:", cos_data)
            

            attn_weight = 4 * attn_weight + cos_data  # 自己写的
            

            # attn_weight = attn_weight + frame_times_param  # 原始的
        ...
        # to_drop_idx = 1  # baseline：限制内存库容量，同时采用固定删除第一帧的方法进行更新
...
class SimplifiedTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        v = _tgt
        tgt2 = self.self_attn(q, k, v, layer_idx=layer_idx)[0]
        ...
        # process_and_visualize(curr_Q, global_K.view(T*token_num, bs, embed_dim), global_V.view(T*token_num, bs, embed_dim))
        ...
        tgt2, attn = self.long_term_attn(
            ...
            is_return_attn_weight=save_atten_weights, layer_idx=layer_idx
        )
        if save_atten_weights and layer_idx == 0:
            bs, head, hw, thw = attn.size()

            # 记录一下 global_K: [T, token_num, bs, embed_dim]，用于 restrict_long_memories 里的冗余性计算
            self.global_K2 = flatten_global_K.view(thw // hw, hw, bs, -1)
            # self.global_K2 = flatten_global_V.view(thw // hw, hw, bs, -1)
...
class DualBranchGPM(nn.Module):
    ...
    def restrict_long_memories(
        ...
        if use_atten_weight:
            ...
            cos_data = 1 / (1 + np.exp(7 * (cos_data - 0.25)))  # 冗余度大于0.25开始极速惩罚
    
            # print("改变后的数据1:", cos_data)
            

            attn_weight = 2 * attn_weight + cos_data  # 自己写的
...
class GatedPropagationModule(nn.Module):
    ...
    def forward(self,
        ...
        cat_tgt2, attn = self.long_term_attn(curr_Q_add_time, flatten_global_K, cat_global_V,
                                          cat_curr_U, size_2d, is_return_attn_weight=save_atten_weights, layer_idx=layer_idx)
        ...
        cat_tgt2, _ = self.self_attn(q, k, v, u, size_2d, layer_idx=layer_idx)

        tgt2, tgt_id2 = torch.split(cat_tgt2, self.d_model, dim=-1)
        # print("self.d_model" + str(self.d_model))
